...
public class OhMyGotto extends GameApplication {
    ...
    private void spawnEnemyWave() {
        if (!GameState.getInstance().isGameStarted()) return;
    ...
    private void initWeapons() {
        ...
        currentWeapon = unlockedWeapons.get("RL");
        updateWeaponHUD();
        /*
        Weapon Types(index):
        Shotgun (SG)
    ...
    private Weapon createWeapon(String id) {
        switch (id) {
            case "SG": return new Weapon(id, 6, 6, 1000 * GameState.getInstance().getPlayerFireRate(), 4 + GameState.getInstance().getPlayerDamageBoost(), 1.5);
            case "SMG": return new Weapon(id, 30, 30, 500 * GameState.getInstance().getPlayerFireRate(), 2 + GameState.getInstance().getPlayerDamageBoost(), 1.2);
            case "AR": return new Weapon(id, 20, 20, 250 * GameState.getInstance().getPlayerFireRate(), 3 + GameState.getInstance().getPlayerDamageBoost(), 2.0);
            case "GL": return new Weapon(id, 3, 3, 1200 * GameState.getInstance().getPlayerFireRate(), 8 + GameState.getInstance().getPlayerDamageBoost(), 1.8);
            case "HG": return new Weapon(id, 12, 12, 400 * GameState.getInstance().getPlayerFireRate(), 2.5 + GameState.getInstance().getPlayerDamageBoost(), 1.5);
            case "SR": return new Weapon(id, 5, 5, 1500 * GameState.getInstance().getPlayerFireRate(), 15 + GameState.getInstance().getPlayerDamageBoost(), 2.8);
            case "RG": return new Weapon(id, 3, 3, 2000 * GameState.getInstance().getPlayerFireRate(), 12 + GameState.getInstance().getPlayerDamageBoost(), 3.0);
            case "MG": return new Weapon(id, 60, 60, 100 * GameState.getInstance().getPlayerFireRate(), 1.8 + GameState.getInstance().getPlayerDamageBoost(), 1.0);
            case "RL": return new Weapon(id, 2, 2, 2000 * GameState.getInstance().getPlayerFireRate(), 10 + GameState.getInstance().getPlayerDamageBoost(), 1.5);
            case "MT": return new Weapon(id, 1, 1, 3000 * GameState.getInstance().getPlayerFireRate(), 25 + GameState.getInstance().getPlayerDamageBoost(), 2.5);
            case "FT": return new Weapon(id, 50, 50, 100 * GameState.getInstance().getPlayerFireRate(), 0.8 + GameState.getInstance().getPlayerDamageBoost(), 0.6);
            default: return new Weapon(id, 10, 10, 500 * GameState.getInstance().getPlayerFireRate(), 3 + GameState.getInstance().getPlayerDamageBoost(), 1.5);
        }
    }
    ...
    private void applyStatUpgrade(String stat) {
        ...
        switch (stat) {
            case "Fire Rate":
            GameState.getInstance().modifyFireRate(0.8);
                break;
            case "Damage":
                GameState.getInstance().addDamageBoost(1.2);
                break;
            case "Move Speed":
    ...
    private void movePlayer(double dx, double dy) {
        ...
        if (!hasMoved && (dx != 0 || dy != 0)) {
            ...
            FXGL.getNotificationService().pushNotification("Game Started!");
        }
    ...
    protected void initUI() {
        ...
        Image cursorImage = FXGL.getAssetLoader().loadImage("crosshair.png");
        ImageCursor crosshair = new ImageCursor(cursorImage, 0, 0);
        FXGL.getGameScene().setCursor(crosshair);
