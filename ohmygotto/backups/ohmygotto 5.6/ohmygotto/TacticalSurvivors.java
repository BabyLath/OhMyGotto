...
public class TacticalSurvivors extends GameApplication {
    ...
    private void spawnEnemyWave() {
        if (!isGamePaused && !isGameOver) {
            ...
            if (playerLevel % 2 == 0 && enemiesPerWave < 20) {
                enemiesPerWave++;
            }
        }
    ...
    protected void initPhysics() {
        ...
        FXGL.getPhysicsWorld().addCollisionHandler(new CollisionHandler(EntityType.PLAYER, EntityType.ENEMY) {
            ...
            protected void onCollisionBegin(Entity player, Entity enemy) {
                ...
                if (isInvincible) return; // Ignore collision if invincibility buff is applied
    ...
    private void applyPowerup(String type) {
        switch (type) {
            case "speed":
                playerSpeed += 100;
                FXGL.runOnce(() -> playerSpeed -= 100, Duration.seconds(5));
                break;
            case "invincibility":
                isInvincible = true;
                FXGL.runOnce(() -> isInvincible = false, Duration.seconds(4));
                break;
            case "magnet":
                isMagnetActive = true;
                FXGL.runOnce(() -> isMagnetActive = false, Duration.seconds(6));
                break;
            case "heal":
                if(FXGL.geti("playerHp") < playerMaxHP) {
                    FXGL.inc("playerHp", + 1);
                    updateHpText();
                }
                break;
    ...
    private void applyMagnetEffect(double tpf) {
        ...
        double baseAttractDistance = isMagnetActive ? 250 : 100;
        double baseSpeed = isMagnetActive ? 150 : 60;
    ...
    private void updateHpText() {
        hpText.setText("HP: " + FXGL.geti("playerHp") + "/" + playerMaxHP);
    }
    ...
    protected void initInput() {
        ...
        FXGL.getInput().addAction(new UserAction("Fire") {
            ...
            protected void onAction() {
                if (!isGameOver && !isGamePaused && currentWeapon != null && currentWeapon.canFire()) {
    ...
    private void movePlayer(double dx, double dy) {
        ...
        player.translate(dx * playerSpeed * FXGL.tpf(), dy * playerSpeed * FXGL.tpf());
    }
    ...
    private void spawnEnemy() {
        ...
        enemy.setProperty("hp", baseEnemyHP + playerLevel * 2);
        enemy.setProperty("speed", 100 + playerLevel * 2);
    }
    ...
    protected void initUI() {
        // Create UI elements
        hpText = FXGL.getUIFactoryService().newText("HP: " + FXGL.geti("playerHp") + "/" + playerMaxHP, Color.WHITE, 24);
